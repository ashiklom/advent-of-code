...
-- NOTE: Jokers (J) are weakest individually
data Rank
  = J | Two | Three | Four | Five | Six
...
longBig :: (Ord a) => [a] -> [a] -> Ordering
longBig x y = case checklengths of
  EQ -> compare x y
  _ -> checklengths
  where checklengths = compare (length x) (length y)

makeGroups :: [Rank] -> [Int]
makeGroups cards
  | J `notElem` cards = doGroups groups
  | all (==J) cards = makeGroups (replicate 5 Two) 
  | maxrank == J = makeGroups [if x == J then maximum cards else x | x <- cards]
  | otherwise = makeGroups [if x == J then maxrank else x | x <- cards]
  where doGroups = List.sortBy (Ord.comparing Ord.Down) . map length
        groups = List.group $ List.sort cards
        longest = List.maximumBy longBig groups
        maxrank = head longest
...
mkHand cards
  ...
  where grouplengths = makeGroups cards
